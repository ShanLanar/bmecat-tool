...
# mit Preisen aus dem ERP (MySQL, Preisliste MERCATEO_PRICE_LIST_NR).
#
...
import os
from config import (
    DIRS, CONNECTIONS, MERCATEO_PRICE_LIST_NR, MERCATEO_CATALOG_XML_PATTERN,
)


def run_update_prices(progress_cb=None, file_progress_cb=None) -> dict:
    """Mercateo-Unite: Preise im BME-1.2-Katalog aus dem ERP aktualisieren."""
    ...
    prices = fetch_price_list(cfg, MERCATEO_PRICE_LIST_NR, progress_cb=p)
    if not prices:
        p("Unite-Preisupdate: keine Preise aus ERP erhalten – abgebrochen.", tag="warn")
        return {}

    return update_prices_in_xml(xml_path, prices, progress_cb=p)
